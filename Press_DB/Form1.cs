    ...
    public partial class Form1 : Form
    {
        ...
        private void StartThread()
        {
            try
            {
                ...
                // 데이터베이스 connectionString 이 가진 주소를 가지고 접속
        ...
        private void opcServerJoin(CancellationToken cancellationToken)
        {
            ...
            receiveItem.Add(opcItems.AddItem("[interface]SC2_rcv.Hoist_Motor_Current", 2)); // WH_LINE
                        ...
                        if (stkState == 0)
                        {
                            ...
                            query = @"
                                    ...
                                    WHERE 
                                        ...
                                        AND tc.State = 'EMPTY'
                                        AND tr.Cell IS NULL
                                    ORDER BY tc.Level ASC, tc.Bay ASC, tc.Bank ASC;
                                    ";
                            ...
                            using (SqlDataReader innerreader = command.ExecuteReader())
                            {
                                ...
                                for (int j = 0; j < cellType.Count; j++)
                                {
                                    ...
                                    // 팔레트 타입이 3730이고 cellType 에 3730 값이 존재하면.
                                    if (pal_type == 3730 && cellType.Contains(pal_typ) == true)
                                    {
                                        // value 가 pal_type 과 같을 시 
                                        if (value == pal_type)
                                        {
                                    ...
                                    // 팔레트 타입이 3730이고 cellType 에 3730 값이 존재하지 않으면.
                                    else if (pal_type == 3730 && cellType.Contains(pal_typ) == false)
                                    {
                                        // value 가 pal_type 과 같거나 클 시
                                        if (value >= pal_type)
                                        {
                                            // itemValues 딕셔너리의 Cell 키의 Value 값에 j 번 째 Cell 을 대입합니다.
                                            itemValues["Cell"] = cell[j];
                                    ...
                                    // 팔레트 타입이 3730이 아니고 value 가 3730 값이 아니면서 pal_typ 이 cellType 에 존재하면.
                                    else if (pal_type != 3730 && cellType.Contains(pal_typ) == true && value != 3730)
                                    {
                                        // value 가 pal_type 과 같을 때.
                                        if (value == pal_type)
                                        {
                                            // itemValues 딕셔너리의 Cell 키의 Value 값에 j 번 째 Cell 을 대입합니다.
                                            itemValues["Cell"] = cell[j];
                                    ...
                                    // 팔레트 타입이 3730이 아니고 value 가 3730 값이 아니면서 pal_typ 이 cellType 에 존재하지 않으면.
                                    else if (pal_type != 3730 && cellType.Contains(pal_typ) == false && value != 3730)
                                    {
                                        // value 가 pal_type 과 같거나 클 때.
                                        if (value >= pal_type)
                                        {
                                            // itemValues 딕셔너리의 Cell 키의 Value 값에 j 번 째 Cell 을 대입합니다.
                                            itemValues["Cell"] = cell[j];
                ...
                for (int i = 0; i < 8; i++)
                {
                    ...
                    if (reader.Read())
                    {
                        ...
                        if (stkState == 0)
                        {
                            ...
                            if (jobLine == 201 || jobLine == 202 || jobLine >= 401 && jobLine <= 406)
                            {
                                if (!string.IsNullOrEmpty(pltNumber))
                                {
                                    // t_Cell 의 Level 오름차순을 기준으로 Bank 값이 searchValue x 2 한 값 또는 2를 곱한 후 1을 뺀 값 그리고 PLT_CODE 앞자리 번호가 동일하며
                                    ...
                                    WHERE 
                                        ...
                                        AND tc.Pal_no = @pltNumber
                                        AND tc.State = 'INCOMP'
                                        AND tr.Cell IS NULL
                                    ORDER BY tc.Prod_time ASC, tc.Udate + tc.Utime ASC;
                                    ";
                                }
                                else
                                {
                            ...
                            command.Parameters.AddWithValue("@searchValue", searchValue);
                            command.Parameters.AddWithValue("@code", code);
                            command.Parameters.AddWithValue("@pltNumber", pltNumber);
        ...
        private bool sendTable()
        {
            try
            {
                ...
                OPCItem writeItem = receiveItem.Find(item => item.ItemID == "[interface]SC2_rcv.Hoist_Motor_Current"); // WH_LINE
                if (writeItem != null)
                {
